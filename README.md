...
### V3.4.7（2026-09-13，定版）
- **直播黑屏修复**：解析 M3U 的 `http-user-agent` / `http-referer` / `http-origin` 透传给播放器，需特定 UA 的源（如 CCTV）恢复正常播放。
- **点播封面秒出**：搜索改用 `ac=videolist` 取完整字段；部分源缺封面时自动回退 `ac=search`。
- **纯直播源导入**：`isTvboxConfig` 现可识别仅含 `lives[]` 的源（无需空 `sites`）。
- **搜索页源条压矮对齐**：高度 38→30px、去顶部 8px 间距与卡片对齐、间距 5→4px。
