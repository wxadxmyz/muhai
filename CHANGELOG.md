...
> 改为 **universal 单包**（移除 `--split-per-abi`）并通过 gradle `ndk.abiFilters` **只保留 arm64-v8a（新机型）+ armeabi-v7a（老机型），剔除 x86 / x86_64 模拟器架构**：一个 APK 覆盖新旧机型，一次下载安装即可，且比全 ABI universal 更省体积。
