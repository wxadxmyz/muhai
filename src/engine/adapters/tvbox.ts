...
function spiderField(v: any): { spider?: string; spiderUrl?: string; spiderMd5?: string } {
  ...
  if (/^https?:\/\//i.test(v)) {
    const parts = String(v).split(';md5;');
    const url = stripMd5(parts[0]); // 去掉可能存在的多余 ;md5; 段
    const md5 = parts[1];
    if (md5) return { spiderUrl: url, spiderMd5: md5 };
...
async function collectSpiders(cfg: SourceConfig): Promise<SourceConfig[]> {
  ...
  if (nonCspNeedsShared && manager) {
    if (manager.spider) {
      sharedCode = manager.spider;
    } else if (manager.spiderUrl) {
      try {
        sharedCode = await fetchText(resolveUrl(manager.spiderUrl, cfg.baseUrl)); // 预先取回共享蜘蛛，避免每站点重复拉取
      } catch {
        sharedCode = null;
      }
    }
