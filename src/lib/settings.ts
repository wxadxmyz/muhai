...
export interface AppSettings {
  ...
  danmakuStyle: { size: number; color: string; opacity: number; speed: number; area: 'full' | 'top' | 'bottom'; outline: boolean };
  // v1.2.1 新增
 ...
 *  A11：默认只放 2 字以上的明确海外词，不再用单字（"美/韩"会误杀纯中文片名如"美好的生活"）。 */
export const DEFAULT_BLOCKLIST = [
  '海外', '韩剧', '美剧', '日剧', '泰剧', '英剧', '法剧', '韩综', '美综', '日综',
  '印度剧', '欧美', '日韩', '港剧', '台剧', '澳剧', '好莱坞',
];
...
const DEFAULTS: AppSettings = {
  ...
  // size 弹幕字号 px；opacity 不透明度 %；speed 飘屏速度档(1~10，5=基准)；area 显示区域：全屏/顶部/底部
  danmakuStyle: { size: 18, color: '#ffffff', opacity: 100, speed: 5, area: 'full', outline: true },
