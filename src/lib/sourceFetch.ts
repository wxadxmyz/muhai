...
function parseFetched(text: string, url: string): FetchResult {
  ...
  if (trimmed.startsWith('[') || trimmed.startsWith('{')) {
    try {
      ...
      // 影视仓 / TVBox 聚合配置：整体作为 1 个 tvbox 源导入，
      // 由 createTvboxSource 在搜索/播放时自动展开 sites 中的 drpy 与苹果CMS 子站。
...
export function parsePasted(text: string): { sources: any[]; error?: string } {
  ...
  try {
    ...
    // TVBox 聚合配置（含 sites/urls/lives）请通过「配置地址」入口导入 URL，
    // 粘贴 JSON 文本无法作为可再次拉取的聚合源使用。
    if (isTvboxConfig(data)) {
      return {
        sources: [],
        error:
          '检测到 TVBox/影视仓聚合配置，请通过上方「配置地址」输入链接导入，以整体作为 1 个聚合源。',
      };
    }
    const valid = normalize(toSourceList(data));
