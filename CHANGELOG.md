...
### 修复一：drpy 源「能搜到、点进去没集数 / 未取到可播放地址」

- **真凶是 V3.7.0 A1 的引擎并发改造，不是 V3.8.4**。V3.6.9 及之前 `drpy3.rs` 是**单 worker**：同源的 init/search/detail/play 都在同一个 QuickJS 上下文里顺序执行，源状态天然连续，所以能播。
- V3.7.0 A1 改成 **4 worker 池 + 全局 round-robin 轮转分发**后，同源的 detail 与 play 会被打散到不同 worker；每个 worker 持有**独立的 QuickJS Runtime/Context，源状态与缓存不跨线程共享**，play 在冷上下文里拿不到 detail 阶段建立的状态 → 没集数 / 没地址。
- 这也解释了此前「探针全过、真机全挂」：探针在单个引擎实例上顺序调用（等于粘性单线程），真机被轮转打散。
- **修复**：`drpy3run` 分发改为**按源 key 粘性哈希**（djb2 → `hash(key) % pool.len()`）。同一源恒定落在同一 worker，单源行为回到 V3.6.9；不同源仍按哈希分散在多 worker 上，多源并发收益完整保留。

### 修复二：播放返回后搜索子站被重置

- **真凶是返回键栈缺前台判断**。搜索页挂载时压入的「分级返回」处理器（子站 → 全部），在点影片进播放页后**仍在栈里**（搜索页并未卸载）。
- 于是播放页按返回键被它**抢先消费**：暗中把子站重置回「全部」并返回「已消费」，播放页却没关；用户再按一次才真正退出，回到搜索页时子站已被重置。
- **修复**：该处理器在 `player.getState().current` 非空（正在播放）时直接放行，交还给播放页的返回逻辑。一次返回直接回搜索页，子站与结果原样保留。

### 修复三：原生弹窗按钮中文化（OK → 确定）

- `alert()` 的按钮文字由 WebView 决定，真机恒为英文 **OK**，代码无法改动。
- 新增 `src/lib/dialog.ts` 的 `showAlert()`：应用内浮层弹窗，按钮为**「确定」**，内容可选中、可长按复制（诊断报告 / 报错日志正是靠这点在剪贴板不可用时兜底）。
- 替换全部 **11 处** alert：`DebugPanel.tsx` 2 处（诊断报告、报错日志）+ `SettingsModal.tsx` 9 处（导入/导出/云备份等提示）。
- 4 处 `window.prompt`（导入音源 JSON / 分享码 / 配置 JSON）为带输入框弹窗，重写成本较高，本版暂不改。

### 修复四：界面文案「音源」→「影视源」（共 24 处）

- 本影视 App 残留大量音乐时代/通用模板的「音源」说法，统一改为「影视源」。
- 涉及 6 个文件：`SourceManager.tsx`(8)、`SettingsModal.tsx`(7)、`VideoPlayer.tsx`(2，含报错提示「该影视源未返回可播放地址…」)、`AddSourceModal.tsx`(1)、`DebugPanel.tsx`(1)、`alistClient.ts`(2)、`sharecode.ts`(1)、`ImportSourcePage.tsx`(1)。
- 有音乐分支的位置（`SettingsModal.tsx` 的 `isMusic` 三元、`ImportSourcePage.tsx` 的 `mediaType` 三元）**只在影视分支改动**，音乐分支保留「音源」，不误伤。
- 顺带把源管理页说明中残留的「支持音乐 JSON」改为「影视站(苹果CMS) / 直播源 / 云盘(alist)」。
- 已核查：影视路径与所有组件中**无「歌曲/专辑/歌手」等其它音乐遗留词**。

> 沿用打包方式：单包仅含 `arm64-v8a`（新机型）+ `armeabi-v7a`（老机型），剔除 x86/x86_64 模拟器架构。

---

## V3.8.7

本版按用户反馈逐条查证历史版本后修复 5 项，核心是**恢复被后续版本改坏的既有修复**。

### 关键查证结论

- 用户反馈的两个「以前修好过」的版本确实存在：**V3.6.5**（播放根治·本地流式代理）与 **V3.6.7**（修 drpy「搜不了影视 / 点进去没集数」）。二者均被后续版本改坏：V3.7.5 #2 改坏 HLS 缓冲，V3.8.4 改坏 drpy 播放兜底。
- V3.6.9 的链路诊断报告实测：量子源（lzcdn27）**实际下载速度约 76 KB/s（600 kbps）**，而片源为 2000k 码率需 250 KB/s —— 上游 2xx 成功 7/7、失败 0，证明**软件链路完全正常**，「播两秒卡一下」的主因是源站 CDN 带宽不足（360 资源快即因其 CDN 快），非软件缺陷。

### 修复

1. **搜索页返回子站重置**（`SearchView.tsx`）：V3.7.4 #2 的 `sessionStorage` 恢复逻辑依赖 `p.kw === initialQuery` 比对，而 `initialQuery`（外层 `searchQuery`）在搜索页内改词重搜时不会同步更新，导致条件恒不成立、子站被打回「全部」。改为有 pref 即直接恢复，并在 `run()` 内同步 pref 的关键词。
2. **起播重复加载**（`VideoApp.tsx`）：详情返回后无条件再次 `playEpisode`，而源站 `play()` 每次返回不同 CDN 路径与签名，第二次起播会丢弃第一次已下好的 manifest 与首片（诊断实测两条「开始加载」相隔约 2 秒）。改为已在播（地址已解析）时只补元数据、不重新起播。
3. **master 清单重复请求**（`hlsPlayer.ts`）：同一 video + 同一 URL 已在加载时直接复用，避免 effect 重跑导致 detach 后重新 `loadSource`，省掉一次 manifest 往返（诊断实测同一 142B 清单同毫秒被请求 2 次）。
4. **HLS 缓冲策略还原默认**（`hlsPlayer.ts`）：彻底移除 V3.7.5 #2 引入的 `startLevel`/`maxBufferLength`/`maxMaxBufferLength`/`backBufferLength` 等自定义参数，恢复 V3.6.5 的 `new Hls({ loader, pLoader: loader })`，缓冲交还 hls.js 自行管理。
5. **drpy 播放兜底回退**（`drpy3.ts`）：删除 V3.8.4 在 `play()` 前加的 `if (!isHttpUrl(playId)) throw` 提前拦截 —— 它堵死了 V3.6.7 修复④ 的核心路径（单集片 detail 无地址时把 **vod_id 直传 `play()`** 让源现算），导致「能搜到、点进去没集数 / 未取到可播放地址」复发。保留 play 返回非 URL 时的二次兜底与最终明确报错（`guardPlayable` 不变）。

> 沿用打包方式：单包仅含 `arm64-v8a`（新机型）+ `armeabi-v7a`（老机型），剔除 x86/x86_64 模拟器架构。
>
> 说明：「播两秒卡一下」的主因是源站 CDN 带宽不足，本版未做改动（软件无法根治，换源或使用 360 类快源可解决）。

---

## V3.8.6

本版修复 V3.7.5 引入的**播放回归**：部分子站点开一直转圈、加载慢，且播放中频繁「播两秒卡一下」（用户体感为「不能播放」）。drpy 源搜索能跑通，说明规则识别与引擎正常，根因在播放端的 HLS 缓冲策略与地址解析错误处理。

### 根因（播放行为被改）

- **V3.7.5 #2**：`hlsPlayer.ts` 把 HLS 向前缓冲上限从 hls.js 默认约 600s 砍到 `maxMaxBufferLength:60`、前向缓冲锁 `maxBufferLength:40`。本地媒体代理是逐分片流式转发，上游一抖，被压扁的缓冲头很快被播完又填不满 →「起播一直转圈、播两秒卡一下」。这是 V3.7.5 之后播放体感变差的直接原因。
- `js.ts` 遗留路径对「拿不到播放地址」**静默返回空 url**，播放器只能落到一句模糊的「未取到可播放地址」，无法区分真实原因。

### 修复

- `hlsPlayer.ts`：回退过激的缓冲收缩——`maxMaxBufferLength` 60→600、`maxBufferLength` 40→60，保留后台裁剪 `backBufferLength:30` 控内存，重试次数 4→6。慢代理源可攒足预读头，直接治「播两秒转圈」与起播慢。
- `js.ts`：遗留路径拿不到地址时**明确抛错**（含常见原因提示），不再静默返回空，真机反馈即可精准定位。
- `drpy3.ts`：`isDrpyRule` 增加 `meta/rule` 对象特征识别，避免 `export default` 形态被 catvodize 剥掉 `export` 后漏判、误派旧沙箱。

> 沿用 V3.8.5 的打包方式：单包仅含 `arm64-v8a`（新机型）+ `armeabi-v7a`（老机型），剔除 x86/x86_64 模拟器架构。
...
> 改为 **universal 单包**（移除 `--split-per-abi`）并通过 `tauri android build --target aarch64 --target armv7` **只编译新机型（arm64-v8a）+ 老机型（armeabi-v7a），剔除 i686 / x86_64 模拟器架构**：一个 APK 覆盖新旧机型，一次下载安装即可，且比全 ABI universal 更省体积。
